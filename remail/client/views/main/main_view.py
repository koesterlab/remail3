...
def create_main_view(page: ft.Page, global_state: AppState):
    ...
    container = ft.ResponsiveRow(expand=True, controls=[
        ft.Column([
            ft.Container(selection_bar, expand=1),
            chatbot
        ], col={"xs": 6, "md": 4, "lg": 3}),
        right_view])
    ...
    def on_thread_change(new: ThreadPreviewDTO|None)->None:
        if new:
            current_conversation = next(filter(lambda conv: new in conv.threads, main_state.get(MainAppStateProperties.DISPLAYED_MAILS)))
