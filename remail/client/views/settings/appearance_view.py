...
class AppearanceView(SettingsSubView):
    def _apply_theme_mode(self, value: str) -> None:
        try:
            # Gelen değeri Flet'in anlayacağı enum formatına çevirmeyi deniyoruz
            theme_mode = ThemeMode(value.lower())
        except Exception:
            # Eğer 'SYSTEM' gibi geçersiz bir değer gelirse çökme, direkt 'dark' yap
